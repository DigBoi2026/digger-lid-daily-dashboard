...
var DLmeta = (function () {
  ...
  /* Read against the live account (Sep 2026): the creative-testing ad sets
     carry the product in the name, with the team's own shorthand — "PRO ENCL",
     "PR MAT", the [PM] [M] [G] [C] prefixes — and the battery grease-gun
     brands (Dewalt, Milwaukee) stand for the KAJO adapter. MOF / BOF / "Mid"
     are the team's words for the warm middle of the funnel. */
  const LINE_RULES = [
    ['Coupler',          /coupler/],
    ['Draw Bar',         /draw ?bar/],
    ['Diggershield',     /digger ?shield|rear screen|earthmover/],
    ['Pro Enclosure',    /enclosure|pro ?encl|topless/],
    ['Pro Mats',         /pro ?mat|promat|(^|[^a-z])pr ?mat\b|\[pm\]|\[m\] /],
    ['Grease',           /grease|kajo|adapt[oe]r|hammer paste|dewalt|milwaukee|makita|\[g\] /],
    ['Excavator Covers', /cover|quicky|skid ?steer|mini ?loader|micro|1\.7|\[c\] /],
    ['Accessories',      /wipes|caddy|cradle|magnet|cap ?set|bottle|beanie|hoodie|trucker|tee\b|hauler|terra ?pro|luggage/],
    /* Whole-catalogue campaigns — Advantage+ shopping, the EOFY and Father's
       Day pushes, giveaways, interest stacks — are Multi/Broad BY DESIGN. They
       count as mapped so the "unmapped" list is left for names nobody can
       place, not for campaigns that were never about one product. */
    ['Multi/Broad',      /\basc\b|nasc|flagship|eofy|\bfd2\d|giveaway|all mid|conversions\/sales|interest|stack/],
  ];
  const TIER_RULES = [
    ['Retention',   /retention|customers?\b|purchasers|existing|loyal|winback|win-back/],
    ['Retargeting', /retarget|rt\b|remarket|warm|dpa|catalog|abandon|viewed|engaged|visitors|\bmof\b|\bbof\b|\bmid\b/],
  ...
  /* Funnel stage from the team's own campaign naming: TOF, TOM, MOF / BOF.
     The campaign name decides; the ad set only when the campaign says nothing.
     MOF and BOF are one stage — both are warm traffic being closed. */
  const STAGES = [['TOF', /\btof\b/], ['TOM', /\btom\b/], ['MOF', /\bmof\b|\bbof\b/]];
  /* The page speaks the team's language — TOF, TOM, MOF/BOF — and the framework
     speaks its own — Prospecting, Retargeting. One map joins them: TOF and TOM
     are cold and judged on the Prospecting benchmarks; MOF/BOF is warm and
     judged on Retargeting's. A row whose names carry no stage word falls back
     on what its audience words said. */
  const STAGE_TIER = { TOF: 'Prospecting', TOM: 'Prospecting', MOF: 'Retargeting' };
  const STAGE_LABEL = { TOF: 'TOF', TOM: 'TOM', MOF: 'MOF / BOF' };
  function stageFor(campaign, adset, tier) {
    return stageOf(campaign, adset) || (tier === 'Prospecting' ? 'TOF' : 'MOF');
  }
  function stageOf(campaign, adset) {
    for (const name of [campaign, adset]) {
      const t = String(name || '').toLowerCase();
      for (const [id, re] of STAGES) if (re.test(t)) return id;
    }
    return null;
  }
  ...
  return { PARAMS, COHORT, FUNNEL, LINES, LINE_RULES, TIER_RULES, STAGES, STAGE_TIER, STAGE_LABEL, classify, stageOf, stageFor, deriveCategory, deriveCoupler, deriveAll, rollup, verdict, streak, fromInsight };
})();
