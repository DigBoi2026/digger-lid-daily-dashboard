...
const DLgpam = (() => {
  ...
  function layers(rows, opts) {
    ...
    return {
      ...
      ads, adsPct: pct(ads.total, netRevenue),
      /* MER on NET revenue — the same base as adsPct and every other rate here.
         Gross sales would include GST (and GST-free export makes the effective
         rate ~8.7%, not 10%), so a gross-sales MER reads ~9% flattering. */
      mer: ads.total ? netRevenue / ads.total : null,
  ...
  function benchmark(rows, anchor, opts) {
    opts = opts || {};
    /* ONE basis for the whole ladder. A day with revenue typed but costs not is
       not a day of GPAM, and the floor already excludes them (monthlySeries drops
       pending). The target must drop them too, or the two halves of one ladder are
       measured on different books and the target reads high. */
    const clean = (rows || []).filter(r => r && !r.pending && (!anchor || r.date <= anchor));
    const series = monthlySeries(clean, opts);
    const full = series.filter(m => m.full && m.L.netRevenue > 0);
    const rates = full.map(m => m.L.gpamPct).sort((a, b) => a - b);
    const all = layers(clean, opts);
    const t12rows = anchor ? clean.filter(r => r.date > addDays(anchor, -365)) : clean;
